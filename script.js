...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      const name = formData.get('name') || formData.get('fullName') || formData.get('full_name') || '';
      const email = formData.get('email') || '';
      const subject = formData.get('subject') || 'Contact Form Submission';
      const message = formData.get('message') || formData.get('msg') || formData.get('comments') || '';
      const phone = formData.get('phone') || formData.get('tel') || formData.get('telephone') || null;
        ...
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '015ad37e-659c-4c7e-87b6-f649244992fc',
                name: name,
                email: email,
                subject: subject,
                message: message,
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "סושי דודי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
